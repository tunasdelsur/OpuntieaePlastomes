...
setwd("F:/....../7_testing_topologies/Trees/TesteDelta2")

read.tree("T1_Nopalea_Early_rooted.tre") -> t1
read.tree("T2_Nopalea_sister_Basilares_rooted.tre") -> t2
...
write.csv(dat.o, "Gene_topology_results_delta_2.csv", row.names=F)
...
write.csv(dat.o, "Gene_topology_results_delta_2_with_alignstats.csv", row.names=F)
...
write.csv(sum.f, "Gene_topology_summary_delta_2.csv", row.names = F)
...
pdf("Gene_topology_delta_2.pdf", width=11, height=8.5)
